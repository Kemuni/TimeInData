...
Tracking activities (work/study/leisure, etc.) every hour to visually view what
takes the most time.
...
![Architecture](readme_asset/architecture_diagram_en.svg)
The architecture of the project is based on the following components:
- **API service** (FastAPI + SQLAlchemy): Provides API which process all data about user actions.
- **Scheduled tasks**: Creates scheduled tasks to message broker, launches with API.
